...
def write_config(tmp_path: Path, overrides: Optional[Dict[str, Any]] = None) -> Path:
    ...
    `overrides` deep-merged into `bridge` only (the only section these
    tests vary), and points `bridge.api_key_env_var` at a real,
    always-set test environment variable so `load_settings()` never
    fails for an unrelated secret-resolution reason."""
