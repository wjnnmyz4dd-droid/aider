...
the only transport since ADR-034 Amendment 10), not fabricated strings,
to prove the classifier reads real production output correctly."""
    ...
    def test_attempt_blocked_and_no_response_all_parsed(self, tmp_path=None):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            log_path.write_text(
                ...
                "2026.07.17 04:41:00.000  Titan (EURUSD,M15)  TITAN_DIAG ATTEMPT route=account transport=HTTP time=2026-07-17T04:41:00+00:00\n"
                "2026.07.17 04:41:00.010  Titan (EURUSD,M15)  TITAN_DIAG NO_RESPONSE transport=HTTP route=account time=2026-07-17T04:41:00+00:00\n",
                encoding="utf-8",
            )
        ...
        self.assertIn(("ATTEMPT", "account", "HTTP"), kinds)
        self.assertIn(("NO_RESPONSE", "account", "HTTP"), kinds)
...
class TestClassifySyntheticEvidence(unittest.TestCase):
    ...
    through a real WebRequest() failure without a real MT5 terminal --
    these use directly-constructed EaEvent/BridgeEvent objects, exactly
    the shape _parse_ea_log/_parse_bridge_console_log already proved
    they produce from real files above."""
    ...
    def test_no_response_is_reported_as_unmatched_not_forced_into_a_state(self):
        """A generic HTTP NO_RESPONSE marker with no pseudoStatus= field
        (see _is_http_pseudo_status_no_response) stays genuinely
        ambiguous rather than being forced into state 2."""
        attempt = self._attempt(transport="HTTP")
        no_response = dc.EaEvent(kind="NO_RESPONSE", route="heartbeat", transport="HTTP",
...
class TestHttpTransportPseudoStatusClassification(unittest.TestCase):
    ...
    "unmatched" the way a generic HTTP NO_RESPONSE (no pseudoStatus) is."""
    ...
    def test_generic_http_no_response_is_still_left_unmatched(self):
        """Guards against the pseudoStatus branch accidentally widening
        to cover a generic HTTP NO_RESPONSE with no pseudoStatus field --
        that case must remain unmatched."""
        ts = datetime(2026, 7, 21, 3, 30, 14, tzinfo=timezone.utc)
        attempt = dc.EaEvent(kind="ATTEMPT", route="heartbeat", transport="HTTP", timestamp=ts,
                              detail="", source_file="ea.log", source_line="ATTEMPT line")
        no_response = dc.EaEvent(kind="NO_RESPONSE", route="heartbeat", transport="HTTP", timestamp=ts,
