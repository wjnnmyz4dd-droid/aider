...
vocabulary, no import from `phantom_pipeline/` or `titan_protocol/bridge/`
(the one permitted upstream import is `titan_protocol.evidence_engine`), no
randomness, in any `titan_protocol/market_intelligence/` source file."""
...
PACKAGE_ROOT = Path(__file__).resolve().parents[3] / "titan_protocol" / "market_intelligence"
...
class TestNoForbiddenImports(unittest.TestCase):
    ...
    def test_only_permitted_import_from_evidence_engine_is_session_and_models(self):
        allowed_evidence_engine_modules = {
            "titan_protocol.evidence_engine.session",
            "titan_protocol.evidence_engine.models",
            "titan_protocol.evidence_engine.config",
        }
        offenders = []
        for path in _source_files():
            tree = ast.parse(path.read_text(), filename=str(path))
            for name in _imported_names(tree):
                if name.startswith("titan_protocol.evidence_engine") and name not in allowed_evidence_engine_modules:
                    offenders.append((path.name, name))
