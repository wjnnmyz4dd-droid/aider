...
real, frozen `titan_protocol/risk_engine/` and `titan_protocol/strategy_engine/`
source.

Confirmed facts (by reading the source, not inferred):
  - `titan_protocol/risk_engine/` contains the substring "regime" nowhere at
  ...
  - `MarketRegime` (`titan_protocol.strategy_engine.models`) has exactly 4
...
from titan_protocol.strategy_engine.models import MarketRegime, QualificationResult, QualificationStatus, StrategyId, StrategySnapshot, WinningStrategy
from tests.titan_protocol.risk_engine._fixtures import T0, make_evidence_snapshot, make_mi_snapshot

RISK_ENGINE_ROOT = Path(__file__).resolve().parents[3] / "titan_protocol" / "risk_engine"
