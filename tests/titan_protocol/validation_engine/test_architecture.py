"""Architecture tests (ADR-030 Hard Rules): no ML/LLM, no execution/
parameter-mutation vocabulary, no import of `phantom_pipeline`/
`titan_protocol.bridge` (the only permitted upstream imports are the six
prior `titan_protocol/` stages' read-only types), statelessness."""
...
PACKAGE_ROOT = Path(__file__).resolve().parents[3] / "titan_protocol" / "validation_engine"
...
FORBIDDEN_IDENTIFIERS = {
    ...
    "mutate_parameter", "apply_recommendation", "modify_config", "reject_trade", "size_position",
}
...
ALLOWED_UPSTREAM_PREFIXES = (
    ...
    "titan_protocol.risk_engine", "titan_protocol.compliance_engine", "titan_protocol.research_engine",
)
FORBIDDEN_ML_MODULES = {"numpy.random", "sklearn", "torch", "tensorflow", "random"}
...
class TestNoForbiddenImports(unittest.TestCase):
    ...
    def test_only_permitted_upstream_packages_are_imported(self):
        ...
        for path in _source_files():
            tree = ast.parse(path.read_text(), filename=str(path))
            for name in _imported_names(tree):
                if name.startswith("titan_protocol.") and not name.startswith("titan_protocol.validation_engine"):
    ...
    def test_no_ml_or_random_imports(self):
        ...
        for path in _source_files():
            tree = ast.parse(path.read_text(), filename=str(path))
            for name in _imported_names(tree):
                if name in FORBIDDEN_ML_MODULES:
                    offenders.append((str(path.relative_to(PACKAGE_ROOT)), name))
        self.assertEqual(offenders, [], f"forbidden random/ML imports found: {offenders}")


class TestStatelessness(unittest.TestCase):
    def test_validation_engine_holds_only_config_and_metrics(self):
        source = textwrap.dedent(inspect.getsource(ValidationEngine.__init__))
    ...
    def test_no_public_method_resembles_a_mutation_or_execution_api(self):
        forbidden_method_names = {"apply", "mutate", "execute", "reject", "approve", "reduce", "select"}
        public_methods = {name for name in dir(ValidationEngine) if not name.startswith("_")}
