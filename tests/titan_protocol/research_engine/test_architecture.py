...
`titan_protocol.bridge` (the only permitted upstream imports are the five
...
PACKAGE_ROOT = Path(__file__).resolve().parents[3] / "titan_protocol" / "research_engine"
...
class TestNoForbiddenImports(unittest.TestCase):
    ...
    def test_only_permitted_upstream_packages_are_imported(self):
        ...
        for path in _source_files():
            tree = ast.parse(path.read_text(), filename=str(path))
            for name in _imported_names(tree):
                if name.startswith("titan_protocol.") and not name.startswith("titan_protocol.research_engine"):
