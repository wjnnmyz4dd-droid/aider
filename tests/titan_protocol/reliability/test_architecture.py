...
anywhere in `titan_protocol/reliability/`, no import of `phantom_pipeline` or
any Runtime *engine* module (only `titan_protocol.runtime.models` and
`titan_protocol.runtime.watchdog_integration` are permitted per ADR-032 Hard
...
PACKAGE_ROOT = Path(__file__).resolve().parents[3] / "titan_protocol" / "reliability"
...
ALLOWED_RUNTIME_MODULES = ("titan_protocol.runtime.models", "titan_protocol.runtime.watchdog_integration")
...
class TestNoForbiddenImports(unittest.TestCase):
    ...
    def test_only_permitted_runtime_modules_are_imported(self):
        ...
        for path in _source_files():
            tree = ast.parse(path.read_text(), filename=str(path))
            for name in _imported_names(tree):
                if name.startswith("titan_protocol.runtime"):
