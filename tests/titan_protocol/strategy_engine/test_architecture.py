...
(the only permitted upstream imports are `titan_protocol.evidence_engine` and
`titan_protocol.market_intelligence`), no randomness.
...
PACKAGE_ROOT = Path(__file__).resolve().parents[3] / "titan_protocol" / "strategy_engine"
...
class TestNoForbiddenImports(unittest.TestCase):
    ...
    def test_only_evidence_engine_and_market_intelligence_are_imported_upstream(self):
        ...
        for path in _source_files():
            tree = ast.parse(path.read_text(), filename=str(path))
            for name in _imported_names(tree):
                if name.startswith("titan_protocol.") and not name.startswith("titan_protocol.strategy_engine"):
