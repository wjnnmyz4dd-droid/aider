...
own approval, and against `titan_protocol.bridge` ever gaining trading-decision
logic of its own."""
...
class TestNoImportsBeyondItsOwnPackage(unittest.TestCase):
    ...
    def test_no_file_imports_a_sibling_titan_protocol_package(self):
        pattern = re.compile(r"^from \.\.(?!bridge\b)[a-z0-9_]+|^from titan_protocol\.(?!bridge\b)[a-z0-9_]+", re.MULTILINE)
        violations = []
        for path in BRIDGE_DIR.glob("*.py"):
            if pattern.search(path.read_text(encoding="utf-8")):
                violations.append(str(path))
        self.assertEqual(violations, [], f"titan_protocol/bridge file(s) import a sibling titan_protocol package: {violations}")
...
class TestGitDiffTouchesNoUnrelatedPackage(unittest.TestCase):
    def test_git_status_shows_only_expected_paths_changed(self):
        ...
        allowed_prefixes = ("titan_protocol/", "mt5/", "tests/titan_protocol/", "docs/research/", "docs/adr/")
