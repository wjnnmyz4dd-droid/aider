"""Architecture tests (ADR-029 Hard Rules): no ML/LLM, no execution/
parameter-mutation vocabulary, no import of `phantom_pipeline`/
`phantom.bridge` (the only permitted upstream imports are the five
...
PACKAGE_ROOT = Path(__file__).resolve().parents[3] / "phantom" / "research_engine"
...
ALLOWED_UPSTREAM_PREFIXES = (
    ...
    "phantom.risk_engine", "phantom.compliance_engine",
)
...
class TestNoForbiddenImports(unittest.TestCase):
    ...
    def test_only_permitted_upstream_packages_are_imported(self):
        ...
        for path in _source_files():
            tree = ast.parse(path.read_text(), filename=str(path))
            for name in _imported_names(tree):
                if name.startswith("phantom.") and not name.startswith("phantom.research_engine"):
...
class TestStatelessness(unittest.TestCase):
    def test_research_engine_holds_only_config_and_metrics(self):
        source = textwrap.dedent(inspect.getsource(ResearchEngine.__init__))
    ...
    def test_no_public_method_resembles_a_mutation_or_execution_api(self):
        forbidden_method_names = {"apply", "mutate", "execute", "reject", "approve", "reduce"}
        public_methods = {name for name in dir(ResearchEngine) if not name.startswith("_")}
