...
`phantom.bridge`/`phantom.evidence_engine` internals beyond `.models`,
...
PACKAGE_ROOT = Path(__file__).resolve().parents[3] / "phantom" / "compliance_engine"
...
ALLOWED_UPSTREAM_PREFIXES = (
    "phantom.evidence_engine", "phantom.market_intelligence", "phantom.strategy_engine", "phantom.risk_engine",
)
...
class TestNoForbiddenImports(unittest.TestCase):
    ...
    def test_only_permitted_upstream_packages_are_imported(self):
        ...
        for path in _source_files():
            tree = ast.parse(path.read_text(), filename=str(path))
            for name in _imported_names(tree):
                if name.startswith("phantom.") and not name.startswith("phantom.compliance_engine"):
...
class TestNeverIncreasesRisk(unittest.TestCase):
    def test_position_sizing_reduction_never_uses_multiplier_above_one(self):
        ...
        from phantom.compliance_engine.config import (
