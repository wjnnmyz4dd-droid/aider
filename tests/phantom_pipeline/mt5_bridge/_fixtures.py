"""Shared test-only fixtures for the MT5 Bridge test suite."""
...
def make_compliance_decision(candidate: CandidateTrade, score_result, risk_decision):
    ...
    account = ComplianceAccountState(
...
def make_execution_decision(candidate, score_result, risk_decision, compliance_decision):
    ...
    return engine.validate(
        ...
        EVBrokerState(connected=True, symbol_tradable=True),
        EVAccountState(equity=10000.0, available_margin=5000.0),
        T0,
        reference_price=PRICE,
        intended_stop_loss=PRICE - STOP_DISTANCE,
        intended_take_profit=PRICE + STOP_DISTANCE * 2,
    )
...
def make_full_chain(trace_id: str = "obs-trace-1"):
    ...
    return candidate, score_result, risk_decision, compliance_decision, execution_decision
