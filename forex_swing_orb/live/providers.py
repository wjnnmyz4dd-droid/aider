...
class DailyAnchorTracker:
    """Persists the equity captured at the first snapshot of each server day. The
    day boundary is operator-configured (FTMO reset); default = UTC date."""

    def __init__(self, path, day_key=None):
        self.path = path
        self._day_key = day_key or (lambda now: now.astimezone(timezone.utc).strftime("%Y-%m-%d"))
        self._anchors = {}
        try:
            with open(self.path, "r", encoding="utf-8") as f:
                ok, obj = serialize.loads(f.read())
            if ok:
                self._anchors = dict(obj.get("anchors", {}))
        except FileNotFoundError:
            pass

    def anchor(self, now, equity):
        key = self._day_key(now)
        if key not in self._anchors:
            self._anchors[key] = float(equity)
            from ..bridge.atomic import atomic_write_text
            atomic_write_text(self.path, serialize.canonical_json({"anchors": self._anchors}))
...
class Mt5AccountStateProvider(AccountStateProvider):
    ...
    def snapshot(self, now):
        ...
        anchor = self.anchor.anchor(now, equity)
        return {
            "balance": float(ai.balance),
            "equity": equity,
            "initial_balance": self.initial_balance,
            "daily_anchor_equity": anchor,
            "current_daily_loss": max(0.0, anchor - equity),
